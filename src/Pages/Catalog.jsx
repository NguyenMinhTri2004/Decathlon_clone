...
import { getProductSimilar } from "../utils/handlegetData";
import Slider from "@mui/material/Slider";

const Catalog = () => {
  const { name } = useParams();

  const [range, setRange] = useState([5, 10000]);
  ...
  const productList = getProductSimilar(name, products);

  const [productResult, setProductResult] = useState([productList]);
    ...
    if (checked) {
      switch (type) {
        case "CATEGORY":
          setFilter({
            ...filter,
            category: [...filter.category, item.catalogslug],
          });
          break;
        case "COLOR":
  ...
  const updateProducts = useCallback(() => {
    ...
    temp = temp.filter((e) => {
      // const check = e.size.find((size) => filter.size.includes(size));
      // return check !== undefined;
      console.log(e.money/100000)
      return e.money/100000 >= range[0] && e.money/100000 <= range[1]
    });
    ...
    setProductResult(temp);
  }, [filter, productList , range[0]]);
  ...
  return (
    <div className="catalog">
      ...
      <div style={{ width: "32rem", padding: "20px" }}>
        ...
        />
        Gía đang nằm trong khoảng : {range[0]} trăm - {range[1]} trăm
      </div>
      <div className="catalog__bottom">
        ...
        <div className="catalog__content">
          <Grid col={4} mdCol={3} smCol={1} gap={5}>
